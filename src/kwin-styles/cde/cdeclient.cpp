/*
 *	$Id: cdeclient.cpp 922431 2009-02-07 01:12:28Z schwarzer $
 *
...
Q3ValueList< CdeClientFactory::BorderSize >
...
CdeButton::CdeButton(CdeClient* parent,
    ...
    : Q3Button(parent->widget(), name), m_btnType(btnType), last_button(Qt::NoButton)
{
...
void CdeButton::drawButton( QPainter* p )
{
	...
	case (BtnHelp):
	    p->setPen( colorGroup.light() );
	    p->drawLineSegments( Q3PointArray(16, helpLLines) );
	    p->setPen( colorGroup.dark() );
	    p->drawLineSegments( Q3PointArray(14, helpDLines) );
	break;
	case (BtnIconify):
	...
	case (BtnClose):
	    p->setPen( colorGroup.dark() );
	    p->drawLineSegments( Q3PointArray(18, closeDLines) );
	    p->setPen( colorGroup.light() );
	    p->drawLineSegments( Q3PointArray(15, closeLLines) );
	break;
    }
}
